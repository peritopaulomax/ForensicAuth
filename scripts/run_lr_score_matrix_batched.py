#!/usr/bin/env python3
"""Batched scorer for LR calibration matrices.

This keeps the same model implementations but avoids one full pipeline call per
image. It writes checkpoints after every batch and supports --resume.
"""
...
def _score_bfree(images: list[Image.Image], output_rows: list[dict[str, Any]], device: torch.device, device_label: str) -> None:
    ...
    for image, out_row in zip(images, output_rows):
        # B-Free keeps native image geometry; process one image at a time because
        # tensors can have different H/W.
...
def _score_corvi(images: list[Image.Image], output_rows: list[dict[str, Any]], device: torch.device, device_label: str) -> None:
    ...
    for image, out_row in zip(images, output_rows):
        ...
        llrs: list[float] = []
        # Corvi keeps full 1024px tiles; microbatching avoids OOM on large images.
        for tile in tiles:
