...
def extract_safe_embedding(image: Image.Image, device: torch.device | None = None) -> np.ndarray:
    ...
    from forensics.safe.safe_pipeline import infer_safe_from_pil
...
def extract_ai_image_detector_embedding(
    ...
    Classification loads this model via Hugging Face ``pipeline()``. Embedding
    extraction calls ``AutoModelForImageClassification`` directly; logits stay
    identical because the classifier still applies to the same pooler output.
    """
    rgb = image.convert("RGB")
...
def extract_bfree_embedding(image: Image.Image, device: torch.device | None = None) -> np.ndarray:
    ...
    from forensics.bfree.bfree_pipeline import infer_bfree_from_pil
...
def extract_clipd_embedding(image: Image.Image, device: torch.device | None = None) -> np.ndarray:
    ...
    from forensics.truebees_clip_d.clipd_pipeline import MODEL_NAME, infer_clipd_from_pil
...
def extract_corvi2023_embedding(
    ...
    from forensics.truebees_clip_d.clipd_pipeline import infer_corvi2023_from_pil
