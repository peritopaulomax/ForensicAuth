"""Pipeline de detecção de imagens sintéticas (ensemble CNN + FFT + Effort + SAFE)."""
...
def _as_rgb(image: Image.Image) -> Image.Image:
    """Converte para RGB — Hugging Face processors rejeitam RGBA."""
    if image.mode == "RGB":
        return image
...
def _load_model4_feature_extractor(model_id: str) -> Any:
    """Carrega AutoFeatureExtractor (fallback AutoImageProcessor) para sdxl-flux-detector_v1.1."""
...
def _hf_detector_logits_and_embedding(
    ...
    *,
    ...
    from forensics.synthetic_image_detection.embedding_utils import flatten_embedding
...
def predict_ensemble(
    ...
    if SYNTHETIC_ANALYSIS_BFREE in selected:
        try:
            if return_embedding:
                ...
                from forensics.bfree.bfree_pipeline import predict_bfree_row
    ...
    if SYNTHETIC_ANALYSIS_CORVI2023 in selected:
        try:
            if return_embedding:
                ...
                from forensics.truebees_clip_d.clipd_pipeline import predict_corvi2023_row
    ...
    if SYNTHETIC_ANALYSIS_SAFE in selected:
        try:
            if return_embedding:
                from forensics.effort.effort_pipeline import effort_row
                ...
                from forensics.safe.safe_pipeline import predict_safe_row
