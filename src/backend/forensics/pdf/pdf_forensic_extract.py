"""Extracao forense de PDF: imagens, metadados, assinaturas e versoes incrementais."""
...
def run_pdf_forensic_extract(
    ...
    if reporter:
        reporter(60, "Analisando assinaturas digitais")

    try:
        signatures_analysis = analyze_pdf_signatures(pdf_path, out_dir)
    except Exception as exc:
        signatures_analysis = {
            "signed": False,
            "signature_count": 0,
            "status": "error",
            "message": f"Falha na analise de assinaturas (extracao continua): {exc}",
            "signatures": [],
            "errors": [str(exc)],
        }

    signatures_json_path = out_dir / "signatures.json"
    signatures_report_path = out_dir / "signatures_report.txt"

    if reporter:
        reporter(72, "Analisando versoes incrementais")
    ...
    manifest = {
        ...
        "signatures": {
            "signed": signatures_analysis.get("signed"),
            "signature_count": signatures_analysis.get("signature_count"),
            "status": signatures_analysis.get("status"),
            "message": signatures_analysis.get("message"),
        },
        "version_files": version_files,
        "metadata_txt": "metadata_report.txt",
        "signatures_json": "signatures.json",
        "signatures_report": "signatures_report.txt",
    ...
    return {
        ...
        "signatures_analysis": signatures_analysis,
        "signatures_json_path": str(signatures_json_path),
        "signatures_report_path": str(signatures_report_path),
        "signature_count": int(signatures_analysis.get("signature_count") or 0),
        "pdf_signed": bool(signatures_analysis.get("signed")),
        "version_files": version_files,
