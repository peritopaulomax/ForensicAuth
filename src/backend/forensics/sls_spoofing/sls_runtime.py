...
def _vendor_root() -> Path:
    return _workspace_root() / "vendor" / "sls_asvspoof"
...
def bootstrap_fairseq() -> None:
    ...
    # Sempre mantem os paths do SLS na frente. Isso evita que outros vendors
    # que tambem usam o nome top-level `model` (ex.: VideoFACT) os ofusquem.
...
def runtime_status() -> Tuple[bool, str]:
    if not _fairseq_root().is_dir():
        return False, "Vendor SLS/fairseq ausente (vendor/sls_asvspoof)"
