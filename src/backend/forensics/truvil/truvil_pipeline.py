"""TruVIL pipeline — trusted video inpainting localization."""
...
from forensics.truvil.truvil_vendor import truvil_vendor_context
...
def clear_truvil_model_cache() -> None:
    _model_cache.clear()
    release_gpu_memory()


def _remap_checkpoint_keys(state: dict) -> dict:
    """Checkpoint usa nomes antigos (tacf/rgd); código atual usa CAF/AND."""
    out = {}
    for key, value in state.items():
        nk = key.replace("encoder.tacf.", "encoder.CAF.").replace("rgd.", "AND.")
        out[nk] = value
    return out


def _patch_hp3d_device(HP3D_cls) -> None:
    """HP3D oficial força CUDA; alinhar peso ao device do input."""
    ...
    HP3D_cls.forward = forward  # type: ignore[method-assign]
...
def _load_model(device: torch.device) -> torch.nn.Module:
    ...
    with truvil_vendor_context():
        import HP3D as hp3d_mod
        from model import TruVIL

        _patch_hp3d_device(hp3d_mod.HP3D)
        model = TruVIL()
        ...
        if isinstance(raw, dict) and "state_dict" in raw:
            ...
            raw = raw["model"]
        state = _remap_checkpoint_keys(dict(raw))
        info = model.load_state_dict(state, strict=False)
        if info.missing_keys or info.unexpected_keys:
            logger.warning(
                "TruVIL load_state_dict missing=%s unexpected=%s",
...
def _write_chart(clip_results: list[ClipResult], out_path: Path) -> None:
    ...
    ax.set_title("TruVIL — localizacao por clip")
    ax.set_ylim(0, 1)
...
def run_truvil_analysis(
    ...
    *,
    ...
    if clip_len != DEFAULT_CLIP_LEN:
        raise ValueError(f"TruVIL requer clip_len={DEFAULT_CLIP_LEN} (AND temporal)")
    ...
    if len(clip_results) > 1:
        chart_path = out_dir / "truvil_scores_chart.png"
...
def write_truvil_report(analysis: TruVilAnalysis, out_dir: str | Path) -> tuple[str, str]:
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    payload = {
        "model": MODEL_LABEL,
        "technique": "truvil",
        "weights_profile": "TruVIL_train_VI_OP",
    ...
    json_path = out_dir / "truvil_report.json"
    txt_path = out_dir / "truvil_summary.txt"
    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    txt_path.write_text(
        (
            f"TruVIL ({MODEL_LABEL})\n"
