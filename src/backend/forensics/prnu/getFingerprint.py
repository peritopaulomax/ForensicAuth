...
def getFingerprint(Images, sigma=3., fromHowMany=-1):
    ...
    t = 0
    ImagesinRP = []
    for i in tqdm (range(database_size)):
        #Fu.SeeProgress(i),
        im = Images[i]
        try:
            img = Image.open(im)
            ...
            print(im +"não é uma imagem válida: ignorando")
            continue        
        X = cv.imread(im);
...
### FUNCTIONS ##
def _double255(X):
    # MATLAB: convert to 'double' in [0, 255].
    # Python port: float32 ('single') for the same numeric range.
