...
REPRODUCIBILITY_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "distildire": {"primary": "distildire_report.json", "profile": "gpu_ml"},
...
def _strip_volatile_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    # #region agent log
    _DEBUG_LOG = Path(__file__).resolve().parents[3] / ".cursor" / "debug-5ac282.log"
    ...
    def _clean(obj: Any, parent_key: str = "root") -> Any:
        if isinstance(obj, dict):
            ...
            for k, v in sorted(obj.items(), key=lambda item: str(item[0])):
                ...
                if not isinstance(k, str):
                    _dbg(
                        "A",
                        "reproducibility.py:_clean",
                        "non-str dict key before endswith check",
                        {"parent_key": parent_key, "key": k, "key_type": type(k).__name__},
                    )
                # #endregion
                ...
                if k == "extract_bundle_dir":
                    continue
                cleaned[k] = _clean(v, parent_key=f"{parent_key}.{k}")
            return cleaned
