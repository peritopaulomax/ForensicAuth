"""ViLocal plugin — video inpainting localization (IEEE SPL 2025)."""
...
class ViLocalPlugin(ForensicPlugin):
    @property
    def name(self) -> str:
        return "vilocal"
    ...
    def validate_parameters(self, parameters: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if clip_len != DEFAULT_CLIP_LEN:
            return False, f"clip_len deve ser {DEFAULT_CLIP_LEN} (protocolo oficial ViLocal)"
        return True, ""
    ...
    def analyze(self, evidence_path: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        ok, reason = vilocal_runtime_status()
        if not ok:
            return {"success": False, "error": reason, "adapter": "vilocal", "status": "unavailable"}
        try:
            out_dir = job_artifact_dir(parameters, fallback_subdir="vilocal_tmp")
            ...
            json_path, txt_path = write_vilocal_report(analysis, out_dir)
            result: Dict[str, Any] = {
                "success": True,
                "adapter": "vilocal",
                "status": "completed",
                ...
                "vilocal_report_json_path": json_path,
                "vilocal_summary_txt_path": txt_path,
                "timestamp": datetime.now(timezone.utc).isoformat(),
            }
            if analysis.scores_chart_path:
                result["vilocal_scores_chart_path"] = analysis.scores_chart_path
            if analysis.overlay_preview_path:
                result["vilocal_overlay_preview_path"] = analysis.overlay_preview_path
            if analysis.mask_preview_path:
                result["vilocal_mask_preview_path"] = analysis.mask_preview_path
            if analysis.heatmap_preview_path:
                result["vilocal_heatmap_preview_path"] = analysis.heatmap_preview_path
            if analysis.input_preview_path:
                result["vilocal_input_preview_path"] = analysis.input_preview_path
            report_progress(on_progress, 100, "Concluido")
            return result
        except Exception as exc:
            return {"success": False, "error": str(exc), "adapter": "vilocal"}
