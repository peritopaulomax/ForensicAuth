...
export default function TruVilAnalysis() {
  ...
  const { status: runtimeStatus } = useTechniqueRuntime("truvil");
  ...
  async function process() {
    ...
    clearMessage();
    await runAnalysis(evidenceId, "truvil", { ...PARAMS }, {
      onArtifactsLoaded: async (jobId) => {
        const parsed = await fetchResultJson<{
          ...
          threshold?: number;
          clips: ClipRow[];
        }>(jobId, "truvil_report.json");
        if (parsed) {
          setReport(parsed);
          setSelectedFrame(parsed.max_start_frame);
        }
        setChartUrl((await fetchImage(jobId, "truvil_scores_chart.png")) ?? null);
        setOverlayUrl((await fetchImage(jobId, "truvil_overlay_preview.png")) ?? null);
        setMaskUrl((await fetchImage(jobId, "truvil_mask_preview.png")) ?? null);
        setHeatmapUrl((await fetchImage(jobId, "truvil_heatmap_preview.png")) ?? null);
        setViewMode("heatmap");
      },
    });
  }
    ...
    <>
      <div style={{ marginTop: "1rem" }}>
        <button
          ...
          style={btnPrimary}
        >
          {running ? "Processando…" : "Analisar com TruVIL"}
        </button>
      </div>
      {running && (
    ...
    <>
      ...
      {report && (
        <div style={{ marginTop: "1rem" }}>
          ...
          {chartUrl && <img src={chartUrl} alt="TruVIL scores" style={{ width: "100%", maxHeight: 220 }} />}
          {currentJobId && (
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginTop: "0.75rem" }}>
              <button
                type="button"
                onClick={() => void saveDerivativeReport("truvil_report.json", "TruVIL — relatório")}
                disabled={saving}
                style={btnSecondary}
              >
              ...
              {heatmapUrl && (
                <button
                  type="button"
                  onClick={() => void saveDerivativeReport("truvil_heatmap_preview.png", "TruVIL — heatmap")}
                  disabled={saving}
                  style={btnSecondary}
                >
                  Salvar heatmap PNG
              ...
              {overlayUrl && (
                <button
                  type="button"
                  onClick={() => void saveDerivativeReport("truvil_overlay_preview.png", "TruVIL — overlay")}
                  disabled={saving}
                  style={btnSecondary}
                >
                  Salvar overlay PNG
              ...
              {maskUrl && (
                <button
                  type="button"
                  onClick={() => void saveDerivativeReport("truvil_mask_preview.png", "TruVIL — máscara")}
                  disabled={saving}
                  style={btnSecondary}
                >
                  Salvar máscara PNG
