...
import { listCaseReferences, uploadReference } from "@/services/evidence";
...
const defaultMatrix: number[][] = libjpegLumaQuantizationMatrix(DEFAULT_LIBJPEG_QUALITY);

type DCTMode = "estimate" | "reference" | "custom";

const MODE_HELP: Record<DCTMode, string> = {
  estimate:
    "Modo 3: estima a matriz de quantização a partir da imagem analisada. Cuidado: nem sempre corresponde à matriz primária original.",
  reference:
    "Modo 1: usa a matriz de quantização de uma imagem JPEG de referência (via jpegio quando possível) e compara com a evidência.",
  custom:
    "Modo 2: aplica uma matriz 8x8 (padrão: libjpeg luminância Q80) e compara com a matriz estimada da evidência. Ajuste a qualidade ou edite os coeficientes.",
};
...
export default function DCTQuantization() {
  ...
  const [customMatrix, setCustomMatrix] = useState<number[][]>(() => cloneMatrix8x8(defaultMatrix));
  /** Qualidade libjpeg que gerou a grade; null = edição manual (fora da tabela padrão). */
  const [matrixQuality, setMatrixQuality] = useState<number | null>(DEFAULT_LIBJPEG_QUALITY);
  const [qualityDraft, setQualityDraft] = useState(String(DEFAULT_LIBJPEG_QUALITY));
  ...
  const { status: runtimeStatus } = useTechniqueRuntime("dct_quantization");
  ...
  async function runDCT() {
    if (!evidenceId || !canProcess() || runtimeOk === false) return;
    ...
    if (mode === "reference") {
      ...
      if (matrixQuality != null) {
        params.jpeg_quality = matrixQuality;
        params.quantization_matrix_source = `libjpeg_luma_q${matrixQuality}`;
      } else {
        params.quantization_matrix_source = "manual";
      }
    }
  ...
  function applyLibjpegQuality(nextQuality: number) {
    const q = clampJpegQuality(nextQuality);
    setMatrixQuality(q);
    setQualityDraft(String(q));
    setCustomMatrix(libjpegLumaQuantizationMatrix(q));
    clearResults();
  }

  function bumpLibjpegQuality(delta: number) {
    const base = matrixQuality ?? clampJpegQuality(parseInt(qualityDraft, 10) || DEFAULT_LIBJPEG_QUALITY);
    applyLibjpegQuality(base + delta);
  }

  function commitQualityDraft() {
    const parsed = parseInt(qualityDraft.trim(), 10);
    if (Number.isNaN(parsed)) {
      setQualityDraft(String(matrixQuality ?? DEFAULT_LIBJPEG_QUALITY));
      return;
    }
    applyLibjpegQuality(parsed);
  }

  function markMatrixManuallyEdited() {
    setMatrixQuality(null);
  }
    ...
    try {
      ...
      const detail =
        err && typeof err === "object" && "response" in err
          ? (err as { response?: { data?: { detail?: string } } }).response?.data?.detail
          : undefined;
      setReferenceError(detail || "Erro ao enviar imagem de referência");
      setReferenceFile(null);
      await loadReferenceCandidates();
    } finally {
  ...
  const processHint =
    mode === "reference" && !referenceEvidenceId
      ? "Envie a imagem de referência antes de processar."
      : !evidenceId
        ? "Selecione uma evidência ou derivado."
        : null;

  const qualityForControls =
    matrixQuality ?? clampJpegQuality(parseInt(qualityDraft, 10) || DEFAULT_LIBJPEG_QUALITY);
  ...
  const parametersPanel = (
    <>
      <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
        {(
          [
            ["estimate", "Estimar matriz de quantização da evidência"],
            ["reference", "Comparar com imagem de referência (upload JPEG)"],
            ["custom", "Matriz de quantização customizada (8x8)"],
          ] as const
        ).map(([value, label]) => (
      ...
      {mode === "reference" && (
        <div style={{ marginTop: "1rem" }}>
          {referenceCandidates.length > 0 && (
            <div style={{ marginBottom: "0.75rem" }}>
              <label style={{ fontSize: "0.8rem", color: "#6b7280", display: "block", marginBottom: "0.25rem" }}>
                Referência já registrada no caso
              </label>
              <select
              ...
              >
                <option value="">Selecione uma referência existente...</option>
          ...
          <p style={{ fontSize: "0.85rem", color: "#374151", marginBottom: "0.5rem", fontWeight: 500 }}>
            Imagem JPEG de referência (matriz primária conhecida ou alegada):
          </p>
          <div
            onDragOver={(e) => {
              e.preventDefault();
          ...
          >
            ...
            />
            {referenceUploading ? (
              <p style={{ color: "#6b7280", fontSize: "0.85rem" }}>Enviando referência...</p>
            ) : referenceFile ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.5rem" }}>
                ...
                <p style={{ color: "#6b7280", fontSize: "0.8rem", margin: 0 }}>
                  {referenceEvidenceId ? "Referência registrada na cadeia de custódia" : "Salvando..."}
                </p>
              </div>
            ) : (
              <div>
        ...
        <div style={{ marginTop: "1rem" }}>
          ...
          >
            <span style={{ fontSize: "0.8rem", color: "#4b5563", fontWeight: 500 }}>
              Qualidade libjpeg (luminância)
            </span>
            <button
              type="button"
              onClick={() => bumpLibjpegQuality(-1)}
              disabled={running || qualityForControls <= 1}
            ...
            <input
              ...
              disabled={running}
              aria-label="Qualidade JPEG 1 a 100"
              style={{
                width: 64,
            ...
            <button
              type="button"
              onClick={() => bumpLibjpegQuality(1)}
              disabled={running || qualityForControls >= 100}
            ...
            <button
              type="button"
              onClick={() => applyLibjpegQuality(DEFAULT_LIBJPEG_QUALITY)}
              disabled={running}
              style={{
            ...
            <span style={{ fontSize: "0.75rem", color: "#6b7280" }}>
              {matrixQuality != null
                ? `Tabela libjpeg Q${matrixQuality}`
                : "Matriz editada manualmente (fora da tabela padrão)"}
            </span>
          </div>
    ...
    <>
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "0.75rem", marginBottom: "0.5rem" }}>
          <h3 style={{ fontSize: "0.9rem", color: "#374151", margin: 0, fontWeight: 600 }}>
            Matrizes de quantização (lado a lado)
          </h3>
          {currentJobId && (
        ...
        >
          <MatrixValuesTable
            ...
            emptyMessage="Matriz estimada indisponível para este resultado."
          />
          <MatrixValuesTable
            matrix={jpegioMatrix}
            title="Matriz lida do arquivo JPEG (jpegio)"
            emptyMessage="jpegio indisponível: evidência não é JPEG ou tabela Q não foi lida."
          />
        </div>
      </div>
      ...
      {typeof result.mean_difference === "number" && (
        <p style={{ fontSize: "0.85rem", color: "#374151" }}>
          Diferença média entre matrizes: <strong>{result.mean_difference.toFixed(2)}</strong>
        </p>
      )}
    </>
  );
  ...
  return (
    <TechniquePageShell
      ...
      runtimeOk={runtimeOk}
      evidenceSelectorProps={{ excludeReferences: true, onLoaded: handleEvidenceLoaded }}
