...
export default function JpegGhostsAnalysis() {
  ...
  const { status: runtimeStatus } = useTechniqueRuntime("jpeg_ghosts");
  ...
  const parametersPanel = (
    <>
      ...
      <p style={{ fontSize: "0.8rem", color: "#6b7280", margin: "0.5rem 0 0" }}>
        Com busca de grade o processamento é mais lento, mas inclui detecção completa de deslocamento.
      </p>
      <div style={{ marginTop: "1rem" }}>
        <button
          ...
          style={btnPrimary}
        >
          {running ? "Processando…" : "Processar JPEG Ghosts"}
        </button>
      </div>
    </>
  );
    ...
    <>
      ...
      {currentJobId && (
        <div style={{ marginTop: "1rem", display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          <button type="button" disabled={saving} onClick={() => save(currentJobId, "ghost_map.png", "Mapa fantasma")} style={btnSecondary}>
            Salvar mapa fantasma
          </button>
          <button type="button" disabled={saving} onClick={() => save(currentJobId, "metric_map.png", "Mapa de metrica")} style={btnSecondary}>
            Salvar metrica
          </button>
        </div>
      )}
          ...
          {currentJobId && (
            <div style={{ marginTop: "0.75rem" }}>
              <button
                type="button"
                disabled={saving}
                onClick={() => save(currentJobId, "quality_montage.png", "Galeria por qualidade")}
                style={btnSecondary}
              >
          ...
          {currentJobId && (
            <div style={{ marginTop: "0.75rem" }}>
              <button
                type="button"
                disabled={saving}
                onClick={() => save(currentJobId, "shift_grid.png", "Grade de deslocamentos")}
                style={btnSecondary}
              >
