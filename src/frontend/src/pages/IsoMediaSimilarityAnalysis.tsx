...
const VIDEO_EXTS = [".mp4", ".mov", ".m4v", ".3gp", ".m4a", ".f4v", ".m4r", ".m4b", ".m4p", ".heic"];

export default function IsoMediaSimilarityAnalysis() {
  ...
  const [caseVideos, setCaseVideos] = useState<Evidence[]>([]);
  ...
  const loadRefGroups = useCallback(
    async (activateRotulo?: string) => {
      ...
      try {
        ...
        const groups = data.groups.filter((g) => g.technique === "isomedia_compare");
        setRefGroups(groups);
  ...
  useEffect(() => {
    ...
    setSelectedRefIds((prev) => {
      const valid = new Set(refVideos.map((r) => r.id));
      if (prev.size > 0) return new Set([...prev].filter((id) => valid.has(id)));
      return valid;
    });
  }, [selectedRotulo, refVideos]);

  const loadCaseVideos = useCallback(async () => {
    if (!caseId) return;
    try {
      const evs = await listCaseEvidences(caseId);
      const videos = filterForensicAuthEvidences(evs).filter((e) => e.file_type === "video");
  ...
  async function handleRefUpload(files: FileList | File[]) {
    ...
    if (!rotulo) {
      setRefUploadMessage({ type: "err", text: "Informe o nome do rotulo antes de enviar videos de referencia." });
      return;
    }
    setUploadingRefs(true);
    setRefUploadMessage(null);
    const list = Array.from(files).filter((f) => VIDEO_EXTS.some((ext) => f.name.toLowerCase().endsWith(ext)));
    if (list.length === 0) {
      setRefUploadMessage({ type: "err", text: "Envie videos ISOBMFF (MP4/MOV/M4V/3GP)." });
    ...
    for (const file of list) {
      ...
      try {
        const ev = await uploadIsomStructureReference(caseId, file, rotulo, (pct) => {
    ...
    if (uploaded.length > 0) {
      ...
      setRefUploadMessage({
        type: "ok",
        text: `${uploaded.length} video(s) no grupo ISO BMFF - ${rotulo} (cadeia de custodia).`,
      });
    }
    setUploadingRefs(false);
  }
  ...
  const canProcess =
    tab === "with_reference" ? selectedRefIds.size > 0 && selectedQuestIds.size > 0 : selectedQuestIds.size >= 2;
  ...
  return (
    ...
    >
      ...
      {tab === "with_reference" && (
        <AnalysisPanel title="Referencias de video (por rotulo)">
          <p style={hintStyle}>
            Envie videos ISO BMFF de referencia agrupados por rotulo. O grupo ativo define quais entram na matriz.
          </p>
          <RotuloPicker
          ...
          />
          <EvidenceDropZone
            inputId="isom-ref-upload"
            accept="video/mp4,video/quicktime,.mp4,.mov,.m4v,.3gp,.m4a,.f4v,.m4r,.m4b,.m4p"
            multiple
            uploading={uploadingRefs}
            disabled={selectedRotulo === "__new__" && !newRotulo.trim()}
            hint="Clique ou arraste videos de referencia"
            subHint={
              activeRotulo
                ? `Grupo: ISO BMFF - ${activeRotulo}`
          ...
          {Object.keys(uploadProgress).length > 0 && <UploadProgressList progress={uploadProgress} />}
          <ReferenceFileList
            items={refVideos}
            ...
            onToggle={toggleRef}
            onSelectAll={() => setSelectedRefIds(new Set(refVideos.map((r) => r.id)))}
      ...
      <AnalysisPanel title={tab === "with_reference" ? "Videos questionados" : "Videos do caso"}>
        <p style={hintStyle}>
          {tab === "with_reference"
            ? "Selecione os videos questionados para comparar com o grupo de referencia."
            : "Selecione ao menos 2 videos; a matriz compara todos contra todos."}
        </p>
        <QuestionedFileList
          items={caseVideos}
          ...
          onSelectAll={() => setSelectedQuestIds(new Set(caseVideos.map((p) => p.id)))}
      ...
      {jaccardUrl && (
        <AnalysisPanel title="Matriz Jaccard">
          ...
          {renderDerivativeActions("similarity_jaccard.png", "isomedia_compare_jaccard", "Salvar matriz Jaccard")}
        </AnalysisPanel>
      )}
      {wlUrl && (
        <AnalysisPanel title="Matriz WL (kernel normalizado)">
          <img src={wlUrl} alt="Matriz WL" style={{ width: "100%", maxWidth: 1100 }} />
          {renderDerivativeActions("similarity_wl_kernel.png", "isomedia_compare_wl", "Salvar matriz WL")}
        </AnalysisPanel>
      )}
      ...
      {selectedRotulo === "__new__" && (
        <label style={fieldStyle}>
          ...
          <input
            ...
            placeholder="Ex.: Encoder_X, Lote_A"
            style={inputStyle}
          />
        </label>
      )}
  ...
  return (
    <div style={{ marginTop: "1rem" }}>
      ...
      {viewMode === "grid" ? (
        ...
        <VideoCheckboxList items={items} selectedIds={selectedIds} onToggle={onToggle} maxHeight={prnuRefListMaxHeight} />
      )}
    </div>
  );
}
...
function QuestionedFileList({
  ...
  if (items.length === 0) {
    return <p style={{ color: "#9ca3af", fontSize: "0.85rem" }}>Nenhum video no caso.</p>;
  }
  return (
    <>
      <FileListViewHeader viewMode={viewMode} onViewModeChange={onViewModeChange}>
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: "0.8rem", color: "#6b7280" }}>
            {selectedIds.size} de {items.length} selecionado(s)
          </span>
        </div>
      </FileListViewHeader>
      {viewMode === "grid" ? (
        ...
        <VideoCheckboxList items={items} selectedIds={selectedIds} onToggle={onToggle} maxHeight={imageSelectorListMaxHeight} />
      )}
    </>
  );
}

function VideoCheckboxList({
  items,
  selectedIds,
  onToggle,
  maxHeight,
