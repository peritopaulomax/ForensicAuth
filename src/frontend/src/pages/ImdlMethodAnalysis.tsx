...
import { getForensicTechniqueMeta, type ForensicTechniqueMeta } from "@/config/forensicTechniqueMeta";
...
export default function ImdlMethodAnalysis({ embedMethodId }: { embedMethodId?: string } = {}) {
  ...
  const { status: runtimeStatus } = useTechniqueRuntime(effectiveMethodId || "imdlbenco");
  ...
  const { saving, saveMessage, save, clearMessage } = useDerivativeSave({
    formatSuccess: (res, label) => `${label} salvo. SHA-256: ${res.evidence.sha256.slice(0, 16)}…`,
  });
  ...
  const applyEvidence = useCallback(
    ...
    [reset, heatmapUrl, scoreMapUrl, overlayUrl, confidenceUrl, clearMessage],
  );
  ...
  async function handleSave(filename: string, label: string) {
    ...
    await save(currentJobId, filename, label, filename === "mask.png" ? effective_parameters : undefined);
  }

  const title = IMDL_METHOD_LABELS[effectiveMethodId ?? ""] || methodMeta?.name || effectiveMethodId || "IMDL";
  const registryMeta = effectiveMethodId ? getForensicTechniqueMeta(effectiveMethodId) : undefined;
  const shellMeta: ForensicTechniqueMeta | undefined = registryMeta
    ? { ...registryMeta, title }
    : methodMeta
      ? {
          title,
          citation: "",
          cardSubtitle: methodMeta.description || "Localização de manipulação via IMDL-BenCo",
          detail: methodMeta.description || "Localização de manipulação via IMDL-BenCo",
        }
      : undefined;
  ...
  const parametersPanel = (
    <>
      {loadingMeta ? (
        ...
        forceDisabled ? (
          <MessageBox type="err" text="Indisponível nesta versão" />
        ) : (
          methodMeta &&
          !methodMeta.ready &&
  ...
  const resultPanel = result?.success !== false && (heatmapUrl || overlayUrl) ? (
    <>
  ...
  return (
    <TechniquePageShell
      caseId={caseId!}
      techniqueId={effectiveMethodId || "imdlbenco"}
      mediaType="imagem"
      embedded={embedded}
