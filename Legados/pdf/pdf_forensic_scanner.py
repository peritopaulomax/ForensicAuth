...
def _build_font_name_to_xref(doc: fitz.Document, resources_xref: int) -> Dict[str, int]:
    out: Dict[str, int] = {}
    try:
        fonts = doc.xref_get_key(resources_xref, "Font")
        if fonts[0] != "xref":
            return out
        fdict_xref = _parse_xref_ref(fonts[1])
        if fdict_xref is None:
            return out
        obj = doc.xref_object(fdict_xref)
        for m in re.finditer(r"/([A-Za-z0-9.#_\-]+)\s+(\d+)\s+0\s+R", obj):
...
def _build_xobject_name_to_xref(doc: fitz.Document, resources_xref: int) -> Dict[str, int]:
    out: Dict[str, int] = {}
    try:
        xo = doc.xref_get_key(resources_xref, "XObject")
        if xo[0] != "xref":
            return out
        xdict_xref = _parse_xref_ref(xo[1])
        if xdict_xref is None:
            return out
        obj = doc.xref_object(xdict_xref)
        for m in re.finditer(r"/([A-Za-z0-9.#_\-]+)\s+(\d+)\s+0\s+R", obj):
            out[m.group(1)] = int(m.group(2))
    except Exception as ex:  # noqa: BLE001
...
class PDFForensicScanner:
    ...
    def _page_resources_xref(self, page: fitz.Page) -> int:
        rk = self.doc.xref_get_key(page.xref, "Resources")
        if rk[0] == "xref":
